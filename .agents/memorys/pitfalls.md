...
- **状态（2026-09-04 已修，D275/router-destroy-guard）**：两判据纯函数 `should_teardown`/`should_deferred_cleanup`（sfu.rs 顶层）+ `room_has_producers` 双姿态 API；leave 路径带 producer 不毁 router，announce 漏斗尾部 producer 全灭时刻补毁 registry/owners。实盘：消费者全关 0 毁房、新会话 T+5s 直接 LIVE（未重启 host）、补毁恰在 device 断连时刻 ×9；计划与证据=主仓 docs/plans/router-destroy-guard/。

...
- 课审补账（09-21 lesson-review）：PIT-206 入册且钉**回归 pin 欠账**（link 修复零测试钉，1c 首刀）；all-gates clippy 行 `--all-targets` 升级确认未落（在册 V 批/1c 附带）；_copy_with_kill 同树守卫属第三处逐入口补钉，下沉 helper 清偿归 V 批。
- **09-24 docking 轮（imgui_viewer 四区改造，子 09d8495/a307036+docs 笔）**：imgui 档案换 `v1.92.9b-docking`（b48d1af，第一方随版 tag；五源文件集逐名相等，URL_HASH 重烘）；shell `ShellOptions{docking opt-in, ini_path}`+`had_saved_layout()`（dummy 无条件禁写）；viewer 拆 app_model/sessions/panels 四层后上 DockBuilder 四区+AppLog 环+reset 钮。三枚对表勘误（F10 闸战果）：`DockBuilder*` 族在 **imgui_internal.h**；`SetNodeSize` 必先于 Split（L3922 官方注：否则比例不精确）；**首版守卫=启动 ini 探测**——网传 `WasActive` 形证伪（该字段属 ImGuiWindow 非 DockNode）。Xwayland 截图基建：`xwd -id <win>` 单窗抓（root 被 Mutter BadMatch 拒/dbus 截图被禁）+ 网络序头解析转 PNG（脚本入主仓计划 evidence/）。门全绿：build/test example · dummy 零 ini · test-cxx 四形 · 首跑 ini 含 5 窗名+DockNode×8。
- **09-24 手测首轮双修（用户实跑揪出）**：① streams_tree 折叠即崩——`TreeNodeEx` 只在展开时压 ID 栈，我无条件渲染子项+`TreePop` = 折叠瞬间 `IDStack>1` 断言（用户点组头 60s 实锤；教训：dummy 无头判据**不覆盖点击路径**，树/菜单类交互件手测单不是走过场）；修=`if(T){子;TreePop();}`。② X11 handler 去重（Xwayland 剪贴板 GetImage BadMatch 风暴良性但 20+条/分钟刷屏，模板观感即"坏了"）——同 code 首播+每百次汇总。
- **09-24 手测第二轮（同族二漏）**：Control 面板 `tiles.empty()` 与 `sel_control<0` 两条早退路径漏 `ImGui::End()`（T4 补丁只配平了函数尾）→ imgui EndChild 断言崩（用户实跑再揪）。修法+教训入册：**给既有函数加 Begin/End 时必须 grep 该函数全部 return**——首修只堵了报错那条，二漏靠全量扫。dummy 判据依旧不覆盖（未登录态无 tile 但 Control 只在 logged_in 渲染=登录前测不到该分支的机检盲区）。

- **09-24 零帧排障战果（根因=车端 SHM 僵尸 publisher，非 SDK）**：用户报 viewer 勾选拉流无画面。排查链全绿后 clincher：streamer stats 帧率塌方（13s 仅 5 帧≈0.4fps）+ 11855 次 `subscriber 重建完成` 自愈风暴 + err `Unable to establish connection to new sender`（iceoryx2 连接表被僵尸 publisher 毒化，subscriber 自愈 4 天不可愈）。处方=C25 清 runtime 后 27.7fps 复产，diag 探针 20s 内 598 帧解码 30fps 0 丢包（native 消费栈无罪实锤，S6 形态 live 首证顺手入账=1c 部分交账）。教训三枚：① **streamer 健康判据=帧率非 bytes_sent**（半速仍增长假象）；② server consumer `paused=false scores=10` 全绿仍可源饿（上游 producer 才是水源）；③ `on_track receiver params codecs=0 mid=""` 系 webrtc-sys 快照常态（S2c 措辞误导本轮重踩）——**真分水岭=receiver stats bytes_received 增量**。新增在册红：fake_drills `drill_diag_disconnect_detection` HEAD 复现红（1a 遗 diag 脚手架 flake=陈旧债名单+1）。误刀已回滚（engine video_receivers 三面摘净，R3c 方向证伪：get_receivers/transceiver.receiver 皆无 ffi 句柄假 wrapper）。恢复处方入 CHANGELOG？否——运维形已有 C25。

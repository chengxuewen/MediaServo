...
- [host] 信令断链感知补全：WebSocket 异常关闭（对端进程被杀/崩溃触发的 RST、读错误）此前不发断链事件，
  车端网关/流播器等下游只能等心跳静默 15 秒才判死；现读错误即时补发断线通知（与正常关闭同权）。
- [host] 多路流共用同一采集源的订阅容量上限：进程内帧总线此前沿用 iceoryx2 默认 8 订阅者上限
  （服务创建时定死），host.yaml 将 8 路流指向同一 source（D282 defaults 合法形态）即有一路
  永远订阅失败，且该服务的旧上限跨重启存活（"重启也不行"）；现每 topic 订阅上限提至 32。
  既有部署恢复：停 host → 清 /tmp/iceoryx2 与 /dev/shm/iox2_* → 起 host（旧服务文件携带旧上限）。

...
pub trait PeerConnectionApi: Send + Sync + 'static {
    ...
    fn on_negotiation_needed<F>(&self, callback: F)
    where
        F: Fn() + Send + Sync + 'static;
    /// W3C onicegatheringstatechange
    fn on_ice_gathering_state_change<F>(&self, callback: F)
    where
        F: Fn(RTCIceGatheringState) + Send + Sync + 'static;
    /// W3C onicecandidateerror
    fn on_ice_candidate_error<F>(&self, callback: F)
    where
        F: Fn(crate::peer_connection::RTCIceCandidateError) + Send + Sync + 'static;
}
